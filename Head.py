...
class Head(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        points = [(self.size // 2 + self.r * cos(radians(self.angle) + 2 * pi * i / self.sides),
                   self.size // 2 + self.r * sin(radians(self.angle) + 2 * pi * i / self.sides)) for i in range(self.sides)]
